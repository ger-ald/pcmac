...
void read_library(void)
{
	...
	for(i = 4; i < Nrofsets; i++)
	{
		...
		name[j] = '\0';
		setptr[i]->stpointer = (struct set *)malloc(sizeof(struct set));
		setptr[i]->stpointer->settype = STRINGSET;
		setptr[i]->stpointer->members = strdup(name);
		if(!(setptr[i]->stpointer->members))
	...
	while(1)
	{
		...
		ststpointer = (struct set **)malloc(Nrofcset * sizeof(struct set *));
		/* Make room for the set symbol pointers. */
		sysypointer = (struct symbol **)malloc(Nrofcset * sizeof(struct symbol *));
